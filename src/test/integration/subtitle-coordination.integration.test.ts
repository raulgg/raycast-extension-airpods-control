...
test("clears the subtitle when the confirmed metadata write fails", async () => {
  // Given
...
test("refreshes all subtitles after a control action without retaining its operation lock", async () => {
  // Given
  createSupportDirectory();
  removeRevisionState();
  mockLaunchCommand.mockResolvedValue(undefined);
...
test("waits for a status read before allowing a control operation to reach the CLI", async () => {
  // Given
  createSupportDirectory();
  removeRevisionState();
  mockLaunchCommand.mockResolvedValue(undefined);
...
test("serializes status snapshots so a later refresh cannot publish before an older one finishes", async () => {
  // Given
  createSupportDirectory();
  removeRevisionState();
  mockLaunchCommand.mockResolvedValue(undefined);
...
test("serializes a standalone reset behind an in-flight control operation", async () => {
  // Given
  createSupportDirectory();
  removeRevisionState();
  mockLaunchCommand.mockResolvedValue(undefined);
...
test("ignores a delayed listening-mode dispatch after a newer confirmed set", async () => {
  // Given
  createSupportDirectory();
  removeRevisionState();
  mockLaunchCommand.mockResolvedValue(undefined);
...
test("ignores a delayed Conversation Awareness dispatch after a newer confirmed toggle", async () => {
  // Given
  createSupportDirectory();
  removeRevisionState();
  mockLaunchCommand.mockResolvedValue(undefined);
...
test("re-reads legacy null refresh contexts instead of clearing a current subtitle", async () => {
  // Given
  createSupportDirectory();
  removeRevisionState();
  mockLaunchCommand.mockResolvedValue(undefined);
...
test("re-reads legacy Conversation Awareness null contexts instead of clearing a current subtitle", async () => {
  // Given
  createSupportDirectory();
  removeRevisionState();
  mockLaunchCommand.mockResolvedValue(undefined);
...
test.each(["missing", "malformed"] as const)("does not accept an old token after %s state recovery", (kind) => {
  // Given
  createSupportDirectory();
  removeRevisionState();
  mockLaunchCommand.mockResolvedValue(undefined);
